...
{
	echo 'رمز عبور غیرمعتبر است. طفا صفحه را رفرش کنید و از دوباره تلاش کنید.';
	exit();
}
...
$folder_name = ABSPATH."/admin/".BKUP_DIR."/";
	...
	for($i = 0;$i < $table_count; $i++)
	{
		...
		if($start < $row_count || $row_count == 0)
		{
			...
			if (!$backup_obj->Execute($path . $filename,$database,$table_name,$start,$end,$row_count))
			{
				...
				$log_error = "خطای جدول بکاپ گیری : ".$table_name.". توضیحات ".$output;
				echo $log_error;
