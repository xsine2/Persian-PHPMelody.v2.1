...
$default_tpl_properties = array(
	...
	'ct_widenav_link' 	=> 'inherit',		//	ul.nav li a.wide-nav-link: color
...
if ($_POST['reset'] == 'true') // AJAX 
{
	...
	//header('Location: '. _URL .'/admin/customize.php');
	exit();
}
...
if ($_POST['action'] == 'customize-save') // AJAX
{
		...
		if ((strtolower($default_value) != strtolower($_POST[$property])) && $_POST[$property] != '')
		{
			...
			switch ($property) 
			{
				...
				case 'ct_widenav_link':
					...
					$customs['ul.nav li a.wide-nav-link']['text-shadow'] = 'none';
					$customs['ul.nav li a.wide-nav-link']['color'] = $value;
	...
	echo json_encode(array('success' => true,
							'msg' => '<div class="alert alert-success"><strong>ذخیره شد!</strong> برگشت به <a href="settings_theme.php">ناحیه مدیریت</a> یا <a href="'._URL.'/index.'._FEXT.'">سایتت را ببین</a>.</div>'
						  ));
...
foreach ($default_tpl_properties as $name => $value)
{
	switch ($name)
	{
		...
		case 'ct_widenav_link':
			$tpl_properties[$name] = ($tpl_customizations['ul.nav li a.wide-nav-link']['color'] != '') ? $tpl_customizations['ul.nav li a.wide-nav-link']['color'] : $value; 
		break;
...
$preview_page = preg_replace('/<a(.*)href="([^"]*)"(.*)>/','<a$1href="javascript:alert(\'لینک در مدت پردازش سفارشی سازی قابل دست یابی نیست.\');"$3>',$preview_page);
...
<div class="row-fluid">
	<div class="span12 ct_title"><?php echo $config['template_f']; ?> سفارشی سازی قالب</div>
</div>
...
<table width="100%" cellpadding="0" cellspacing="0" class="table ct_table">
	...
	<tr>
		<td colspan="2">
			...
			<div align="center"><a href="customize.php?reset=true" id="reset-to-default">همه را به پیش فرض برگردان</a></div>
		</td>
	</tr>
</table>
</form>
...
$(document).ready(function(){
	...
	$("#button-cancel").click(function(){
		window.location = "<?php echo _URL .'/admin/settings_theme.php';?>";
	});
	...
	$("#button-save").click(function(){
		...
		$.ajax({
			url: "<?php echo _URL.'/admin/customize.php';?>",
			data: var_form_data,
			type: "POST",
			dataType: "json",
	...
	$('#reset-to-default').click(function(){
		...
		if (confirm('ایا از اینکه همه مقادیر را می خواهید به پیش فرض برگردانید مطمئن هستید؟')) { 
			$.ajax({
				url: '<?php echo _URL .'/admin/customize.php'?>',
				data: {
					reset: 'true'
				...
				dataType: 'json',
				success: function(data){
					window.location = "<?php echo _URL .'/admin/customize.php';?>";
				}
			});
		}
		return false;
