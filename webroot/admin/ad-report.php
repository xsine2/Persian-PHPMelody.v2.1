...
$load_flot = 1;
$load_datepicker = 1;
$_page_title = 'آمار و ارقام نمایش تبلیغات سایت';
include('header.php');
...
if ($_GET['date_start'] != '' && $_GET['date_end'] != '')
{
	...
	{
		$errors[] = 'تاریخ غیرمعتبر. فرمت مورد قبول به این صورت است : روز/ماه/سال.';
	}
	...
	if (count($errors) == 0)
	{
		...
		{
			$errors[] = 'تاریخ دوم نمی تواند از تاریخ اول کم تر باشد.';
		}
	}
}
else
...
if ($time_start <= $time_end)
{
	...
	{
		$errors[] = 'بازیابی داده ها از دیتابیس موفقیت آمیز نبود.<br /><strong>mysql بر می گرداند :</strong> '. mysql_error();
	}
}
	...
	<h2>گزارشات تبلیغ</h2>
		...
		<div class="enclosed enclosed-quick-add shadow-div border-radius2">
			...
			<form class="form-horizontal" name="graph-input-data" method="get" action="ad-report.php">
			  	<strong>از</strong>: <input type="text" id="datepicker_start" name="date_start" value="<?php echo date('m/d/Y', $time_start); ?>" class="datepicker input-small" data-date-format="mm/dd/yyyy" autocomplete="off">
				<strong>تا</strong>:	<input type="text" id="datepicker_end" name="date_end" value="<?php echo date('m/d/Y', $time_end); ?>" class="datepicker input-small" data-date-format="mm/dd/yyyy" autocomplete="off">
				<strong>تبلیغ</strong>:	<select name="selected-ad">
					<option value="all">همه</option>
					<optgroup label="Classic Ads">
						<option value="classic-all" <?php echo ( ! $selected_ad_id && $selected_ad_type == _AD_TYPE_CLASSIC) ? 'selected="selected"' : ''; ?>>همه</option>
					...
					<optgroup label="Pre-roll Static Ads">
						<option value="preroll-all" <?php echo ( ! $selected_ad_id && $selected_ad_type == _AD_TYPE_PREROLL) ? 'selected="selected"' : ''; ?>>همه</option>
					...
					<optgroup label="Pre-roll Video Ads">
						<option value="video-all" <?php echo ( ! $selected_ad_id && $selected_ad_type == _AD_TYPE_VIDEO) ? 'selected="selected"' : ''; ?>>همه</option>
				...
				<input type="submit" class="btn btn-blue" value="نمایش" name="submit" />
			</form>
			</div>
		</div>
		<hr />	
